...
- Shared build cache keyed by source digest, torch version, CUDA version and compute capability: GPUs of the same capability share builds, and a warmed cache loads without a compiler, CUDA toolkit or ninja. Failed builds are detected and cleaned automatically.
- `scripts/precompile_ops.py` warms the same cache ahead of time, for the current device by default or an explicit `--arch` list.
